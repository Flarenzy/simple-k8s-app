...
export default function App() {
	...
	const username = keycloakEnabled ? keycloak?.tokenParsed?.preferred_username || keycloak?.tokenParsed?.name || "Account" : "admin";
	const navigate = (next: View) => { const allowed = next === "import" && !capabilities.canCreate ? "dashboard" : next; setView(allowed); if (allowed !== "subnet") setSelectedSubnet(null); };
	...
	if (!authReady) return <div className="page page--center"><div className="card"><h1 className="title">Signing you in…</h1><p className="muted">{authError || "Redirecting to Keycloak."}</p></div></div>;
		return <div className="app"><AppHeader view={view} username={username} role={roleLabel(roles)} authenticated={keycloakEnabled && Boolean(keycloak)} canImport={capabilities.canCreate} onNavigate={navigate} onChangePassword={changePassword} onLogout={() => { if (keycloak) void keycloak.logout(); }} />{authError ? <div className="content"><div className="error">{authError}</div></div> : null}{view === "dashboard" ? <DashboardView subnets={subnets} sites={sites} usage={usage} loading={loading} error={error} canCreate={capabilities.canCreate} onSelectSubnet={openSubnet} onAddSubnet={() => setView("subnets")} /> : view === "subnets" ? <SubnetsView subnets={subnets} sites={sites} loading={loading} error={error} canCreate={capabilities.canCreate} canEdit={capabilities.canEdit} canDelete={capabilities.canDelete} onSelect={openSubnet} onSave={saveSubnet} onDelete={deleteSubnet} /> : view === "sites" ? <SitesView sites={sites} loading={loading} error={error} canCreate={capabilities.canCreate} canEdit={capabilities.canEdit} canDelete={capabilities.canDelete} onSave={saveSite} onDelete={deleteSite} onImport={() => setView("import")} /> : view === "import" && capabilities.canCreate ? <ImportView onImport={importCSV} /> : selectedSubnet ? <SubnetDetailView subnet={selectedSubnet} site={sites.find((site) => site.id === selectedSubnet.site_id)} requester={requester} canEdit={capabilities.canEdit} canDelete={capabilities.canDelete} onBack={() => navigate("subnets")} onRefreshUsage={refresh} /> : null}</div>;
}
