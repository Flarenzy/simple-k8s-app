...
export default function App() {
	...
	const loadServiceSummary = useCallback(async (subnetId: number) => {
		setServiceSummaries((current) => ({ ...current, [subnetId]: { count: 0, statuses: {}, state: "loading" } }));
		try {
			const summary = summarizeServices(await api.kubernetesServices(requester, subnetId));
			setServiceSummaries((current) => ({ ...current, [subnetId]: { ...summary, state: "ready" } }));
		} catch {
			setServiceSummaries((current) => ({ ...current, [subnetId]: { count: 0, statuses: {}, state: "unavailable" } }));
		}
	}, [requester]);
