...
LOCAL_KEYCLOAK_ROLE_CLIENT_ID := ipam-api
...
run:
	...
	@bash -c 'trap "kill 0" EXIT; DB_CONN="$(DB_DSN)" PORT=4040 AUTH_ENABLED=true KEYCLOAK_ISSUER="$(LOCAL_KEYCLOAK_ISSUER)" KEYCLOAK_AUDIENCE="$(LOCAL_KEYCLOAK_AUDIENCE)" KEYCLOAK_JWKS_URL="$(LOCAL_KEYCLOAK_JWKS_URL)" go run ./cmd/api & VITE_KEYCLOAK_URL="$(LOCAL_KEYCLOAK_URL)" VITE_KEYCLOAK_REALM="$(LOCAL_KEYCLOAK_REALM)" VITE_KEYCLOAK_CLIENT_ID="$(LOCAL_KEYCLOAK_CLIENT_ID)" VITE_KEYCLOAK_ROLE_CLIENT_ID="$(LOCAL_KEYCLOAK_ROLE_CLIENT_ID)" VITE_API_BASE="/api/v1" npm --prefix frontend run dev -- --host'
...
test-integration:
	go test -count=1 -v -tags=integration ./integration/api/...
